...
describe("Tooltip.vue", () => {
    ...
    test("basic tooltip", async () => {
        ...
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeFalsy();
        ...
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeTruthy();
        expect(wrapper.get(".xr-tooltip__popper").text()).toBe("hello tooltip");
    ...
    test("tooltip with hover trigger", async () => {
        ...
        // 测试悬停显示
        wrapper.find(".xr-tooltip__trigger").trigger("mouseenter");
        await vi.runAllTimers();
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeTruthy();
        // 测试悬外隐藏
        wrapper.find(".xr-tooltip").trigger("mouseleave");
        await vi.runAllTimers();
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeFalsy();
    });
    ...
    test("tooltip with contextmenu trigger", async () => {
        ...
        // 测试右键菜单显示
        wrapper.find(".xr-tooltip__trigger").trigger("contextmenu");
        await vi.runAllTimers();
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeTruthy();
        // 测试右键菜单隐藏（可以模拟点击外部区域）
    });
    ...
    test("disabled tooltip", async () => {
        ...
        // 测试禁用状态下点击不会触发显示
        wrapper.find(".xr-tooltip__trigger").trigger("click");
        await vi.runAllTimers();
        expect(wrapper.find(".xr-tooltip__popper").exists()).toBeFalsy();
    });
